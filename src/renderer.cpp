...
public:
    ...
    Icon(const std::vector<Img> images, std::function<size_t()> getSpriteIndex, uint8_t width = 8, uint8_t scale = 2) : 
        images(images), width(width), height(images[0].first.size()), scale(scale), getSpriteIndex(getSpriteIndex){
        sprite = new TFT_eSprite(&Renderer::tft);
        sprite->createSprite(width*scale, height*scale);
        redraw();
    }

    Icon(const Img image, uint8_t width, uint8_t scale):
        images({image}), width(width), height(image.first.size()), scale(scale), getSpriteIndex([](){return 0;}) {
        sprite = new TFT_eSprite(&Renderer::tft);
        sprite->createSprite(width*scale, height*scale);
        redraw();
    }
